...
const ProfilePage = () => {
  ...
  const courses = profileRef?.courses || [];
  const groups = profileRef?.groups || [];
  const courseProgress = profileRef?.courseProgress || [];
        ...
        <div className={styles.profileHeader}>
          <div className={styles.userInfoCard}>
            ...
            <div className={styles.userDetails}>
              ...
              <p className={styles.userRole}>{user.role || 'N/A'}</p>
              <p className={styles.userEmail}>{user.email || 'N/A'}</p>
              <p className={styles.userPhone}>{user.phone || 'N/A'}</p>
              <div className={styles.userMeta}>
                <span>Age: {profileRef?.age || 'N/A'}</span>
                <span>Gender: {profileRef?.gender || 'N/A'}</span>
        ...
        <div className={styles.profileContent}>
          ...
          <div className={styles.contentSection}>
            ...
            {courses.length > 0 ? (
              ...
              <p className={styles.noData}>No courses enrolled yet.</p>
            )}
          </div>
          ...
          <div className={styles.contentSection}>
            ...
            {groups.length > 0 ? (
                  ...
                  <div key={group._id} className={styles.groupCard}>
                    ...
                    >
                      ...
                      <span className={styles.lectureCount}>
                        {group.lectures?.length || 0} lecture{group.lectures?.length !== 1 ? 's' : ''}
                      </span>
                    </div>
              ...
              <p className={styles.noData}>No groups assigned yet.</p>
            )}
          </div>
            ...
            {courseProgress.length > 0 ? (
              <div className={styles.progressGrid}>
                {courseProgress.map((progress) => {
                  const course = courses.find(c => c._id === progress.course);
                  const attendedLectures = progress.lectureProgress?.filter(lp => lp.attendance === 'present').length || 0;
                  ...
                  const submittedTasks = progress.lectureProgress?.filter(lp => lp.task?.taskStatus === 'submitted') || [];
                  const avgScore = submittedTasks.length > 0
                    ? Math.round(submittedTasks.reduce((acc, lp) => acc + (lp.task?.score || 0), 0) / submittedTasks.length)
                    : 0;
                  ...
                  return (
                    <div key={progress._id} className={styles.progressCard}>
                      ...
                      <div className={styles.progressStats}>
                        ...
                        <div className={styles.stat}>
                          <span className={styles.statValue}>{submittedTasks.length}</span>
              ...
              <p className={styles.noData}>No progress data available yet.</p>
            )}
          </div>

          {/* Detailed Course Progress */}
          <div className={styles.contentSection}>
            <h2>Detailed Course Progress</h2>
            {courseProgress.length > 0 ? (
                      ...
                      {expandedProgress.has(progress._id) && (
                        <div className={styles.progressDetails}>
                          ...
                          {progress.lectureProgress && progress.lectureProgress.length > 0 ? (
                                ...
                                return (
                                  <div key={lectureProgress._id} className={styles.lectureProgressItem}>
                                    <div className={styles.lectureProgressHeader}>
                                      <h5>{lectureDetails?.title || 'Unknown Lecture'}</h5>
                                      <span className={`${styles.attendanceStatus} ${styles[lectureProgress.attendance || 'unknown']}`}>
                                        {lectureProgress.attendance || 'N/A'}
                                      </span>
                                    </div>
                                    ...
                                    <div className={styles.lectureProgressDetails}>
                                      <div className={styles.progressInfo}>
                                        <div className={styles.infoRow}>
                                          ...
                                          <span className={styles.infoValue}>{lectureProgress.engagement || 0}%</span>
                                        </div>
                                        <div className={styles.infoRow}>
                                          <span className={styles.infoLabel}>Task Status:</span>
                                          <span className={`${styles.taskStatus} ${styles[lectureProgress.task?.taskStatus || 'unknown']}`}>
                                            {lectureProgress.task?.taskStatus || 'N/A'}
                                          </span>
                                        </div>
                                        {lectureProgress.task?.taskStatus === 'submitted' && (
                                          <>
                                            <div className={styles.infoRow}>
                                              ...
                                              <span className={styles.infoValue}>{lectureProgress.task?.score || 0}/100</span>
                                            </div>
                                            <div className={styles.infoRow}>
                                              <span className={styles.infoLabel}>Submitted:</span>
                                              <span className={styles.infoValue}>
                                                {formatDateTime(lectureProgress.task?.submittedAt)}
                                              </span>
                                            </div>
                                          </>
                                        )}
                            ...
                            <p className={styles.noLectures}>No lecture progress data available.</p>
                          )}
                        </div>
                      )}
                    </div>
              ...
              <p className={styles.noData}>No detailed progress data available yet.</p>
            )}
          </div>
        </div>
      </div>
